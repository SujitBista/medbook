/**
 * GET /api/admin/departments/[id] - Get department (admin only)
...
async function proxy(
  ...
  const url = `${env.apiUrl}/admin/departments/${encodeURIComponent(id)}`;
...
export async function GET(
  ...
  if (!id) {
    return NextResponse.json(
      { success: false, error: { message: "Department ID required" } },
      { status: 400 }
    );
  }

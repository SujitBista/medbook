...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.log("[AdminHealth] Token generated successfully", {
        userId: session.user.id,
        role: session.user.role,
        tokenLength: token.length,
      });
    } catch (tokenError) {
      console.error("[AdminHealth] Failed to generate token:", tokenError);
      return NextResponse.json(
        {
          success: false,
          error: {
    ...
    try {
      console.log("[AdminHealth] Calling backend API:", {
        url: `${env.apiUrl}/admin/health`,
        hasToken: !!token,
      });
      response = await fetch(`${env.apiUrl}/admin/health`, {
        headers: {
      ...
      console.error(
        "[AdminHealth] Fetch error (backend unavailable):",
        fetchError
      );
      return NextResponse.json(
        {
    ...
    try {
      ...
      console.error(
        "[AdminHealth] Failed to parse backend response:",
        parseError
      );
      return NextResponse.json(
        {
    ...
    if (!response.ok) {
      console.error("[AdminHealth] Backend API error:", {

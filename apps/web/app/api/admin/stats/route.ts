...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.log("[AdminStats] Token generated successfully", {
        userId: session.user.id,
        role: session.user.role,
        tokenLength: token.length,
      });
    } catch (tokenError) {
      console.error("[AdminStats] Failed to generate token:", tokenError);
      return NextResponse.json(
        {
          success: false,
          error: {
    ...
    try {
      console.log("[AdminStats] Calling backend API:", {
        url: `${env.apiUrl}/admin/stats`,
        hasToken: !!token,
      });
      response = await fetch(`${env.apiUrl}/admin/stats`, {
        headers: {
      ...
      console.error(
        "[AdminStats] Fetch error (backend unavailable):",
        fetchError
      );
      return NextResponse.json(
        {
    ...
    try {
      ...
      console.error(
        "[AdminStats] Failed to parse backend response:",
        parseError
      );
      return NextResponse.json(
        {
    ...
    if (!response.ok) {
      console.error("[AdminStats] Backend API error:", {
